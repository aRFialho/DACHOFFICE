...
export type CatalogSyncSummary = { runId: string; status: "completed" | "retryable" | "not_claimed"; pagesSeen: number; itemsSeen: number; mappedCount: number; unresolvedCount: number };
export interface CatalogSyncRunner { run(runId: string): Promise<CatalogSyncSummary>; }
export interface CatalogSyncJob { outboxId: string; idempotencyKey: string; runId: string; leaseId?: string; attemptCount?: number; }
export interface CatalogSyncQueue { claimNext(): Promise<CatalogSyncJob | null>; settle(job: CatalogSyncJob, delivered: boolean): Promise<void>; }

export class TrayCatalogOutboxWorker {
  constructor(private readonly queue: CatalogSyncQueue, private readonly syncService: CatalogSyncRunner) {}
...
type OutboxRow = { id: string; idempotency_key: string; payload_json: unknown; attempt_count: number };
...
export class PostgresCatalogSyncQueue implements CatalogSyncQueue {
  ...
  async claimNext(): Promise<CatalogSyncJob | null> {
    ...
    try {
      ...
      for (;;) {
        ...
        return { outboxId: row.id, idempotencyKey: row.idempotency_key, runId, leaseId, attemptCount: row.attempt_count + 1 };
      }
    } catch (error) {
  ...
  async settle(job: CatalogSyncJob, delivered: boolean): Promise<void> {
    ...
    if (delivered || (job.attemptCount ?? 1) >= MAX_ATTEMPTS) {
      const errorCode = delivered ? "catalog_sync_delivered" : "catalog_sync_retry_exhausted";
    ...
    await this.pool.query(
      `UPDATE outbox_message SET status = 'pending', available_at = now() + ($3 * interval '1 second'),
       ...
       WHERE ${leaseWhere}`,
      [job.outboxId, job.leaseId, delay, JSON.stringify({ errorCode: "catalog_sync_retryable" })],
    );
  }
}