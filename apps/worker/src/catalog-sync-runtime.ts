...
import {
  ...
  type TrayRefreshTransport,
} from "../../../packages/catalog/src/tray-credential-provider.js";
import {
...
class PostgresTrayConnectionRepository {
  ...
  async loadEncrypted(connectionId: string) {
    const result = await this.pool.query<{
      ...
      FROM tray_store_connection WHERE integration_id = $1`,
      [connectionId],
    );
    ...
    return {
      ...
      accessToken: {
        ciphertext: row.access_token_ciphertext.toString("base64"),
        iv: row.access_token_iv.toString("base64"),
        authTag: row.access_token_auth_tag.toString("base64"),
      },
      refreshToken: {
        ciphertext: row.refresh_token_ciphertext.toString("base64"),
        iv: row.refresh_token_iv.toString("base64"),
        authTag: row.refresh_token_auth_tag.toString("base64"),
      },
  ...
  async replaceEncryptedTokens(input: {
    ...
    await this.pool.query(
      `UPDATE tray_store_connection SET access_token_ciphertext = $2, access_token_iv = $3, access_token_auth_tag = $4,
...
export const createConcreteCatalogSyncWorker = (input: {
  pool: Pool;
  ...
  const service = new CatalogSyncService({
    ...
    provider: (run) =>
      new TrayCatalogAdapter({
        connectionId: run.integrationId,
        credentials,
        fetch: input.fetch,
        timeoutMs: 10_000,
        rateBudget: new ProcessRateBudget(),
      }),
  });
  return new TrayCatalogOutboxWorker(
    new PostgresCatalogSyncQueue(input.pool),
    service,
  );
};
