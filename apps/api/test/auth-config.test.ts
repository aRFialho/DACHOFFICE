...
describe('auth runtime configuration', () => {
  it('fails closed when the JWT secret is missing', () => {
    expect(() => loadAuthRuntimeConfig({ OFFICE_DATABASE_URL: environment.OFFICE_DATABASE_URL })).toThrow(
      'JWT_ACCESS_SECRET is required',
    );
  });

  it('uses short-lived access tokens and secure cookies in production', () => {
    expect(loadAuthRuntimeConfig({ ...environment, NODE_ENV: 'production' })).toEqual({
