...
describe("AgentLifecycleService", () => {
  it("appends a new immutable version instead of overwriting the active one", async () => {
    await expect(
      new AgentLifecycleService(repository).appendVersion(version),
  ...
  it("permits only documented lifecycle transitions", async () => {
    ...
    await expect(
      service.transition("agent-1", "draft", "active", "user-1"),
    ).resolves.toBeUndefined();
    await expect(
      service.transition("agent-1", "draft", "suspended", "user-1"),
    ).rejects.toThrow("lifecycle transition is invalid");
  ...
  it("fails when a hard revocation does not match an active grant", async () => {
    ...
    await expect(
      service.revokeGrant("agent-1", "grant-1", "user-1"),
    ).rejects.toThrow("active grant was not found");
  });
});
