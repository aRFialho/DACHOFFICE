...
import { createAuthService, hashPassword, InMemoryAuthRepository } from '../src/modules/auth/index.js';
...
const payload = {
  ...
  schedules: [{ weekday: 1, workStart: '08:00', workEnd: '17:00', timezone: 'America/Sao_Paulo', onCall: false }],
...
describe('Agent Forge endpoint', () => {
  ...
  it('requires an authenticated Admin Master', async () => {
    ...
    const response = await server.inject({ method: 'POST', url: '/v1/admin/agents', payload });
