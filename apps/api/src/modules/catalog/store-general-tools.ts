...
import { defineTool, type RuntimeSchema, type ToolAuthorizationDecision } from "../tools/tool-contracts.js";
import { ToolRegistry } from "../tools/tool-registry.js";

export type CanonicalProduct = { id: string; sku: string; name: string; active: boolean };
export type CanonicalCost = { productId: string; cost: string; currency: string; observedAt: string };
...
export interface CatalogReadRepository {
  ...
  getListing(sku: string): Promise<CanonicalListing | { status: "unresolved" } | null>;
}
...
const textSchema = (field: "sku" | "query"): RuntimeSchema<{ [key: string]: string }> => ({
  parse(value) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) return { ok: false };
    const text = (value as Record<string, unknown>)[field];
    if (typeof text !== "string" || text.trim().length === 0 || text.trim().length > 160) return { ok: false };
    return { ok: true, value: { [field]: text.trim() } };
  },
});
const outputSchema: RuntimeSchema<unknown> = { parse: (value) => ({ ok: true, value }) };
...
export const createStoreGeneralTools = (repository: CatalogReadRepository) => {
  ...
  const invoke = async (toolCode: string, input: unknown, context: PolicyEvaluationContext): Promise<ToolResult> => {
    ...
    if (toolCode === "products.search") return { status: "found", products: await repository.search(text.query!) };
