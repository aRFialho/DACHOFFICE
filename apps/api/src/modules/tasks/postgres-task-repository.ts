  ...
  async createHumanTask(
    ...
    try {
      ...
      await client.query(
        ...
        [taskId, input.officeId, input.type, input.title, input.description, input.priority, input.requestedByUserId, createdAt],
      );
      ...
      await client.query(
        ...
        [eventId, taskId, input.requestedByUserId, JSON.stringify({ source: input.source, priority: input.priority })],
      );
      await client.query(
        ...
        [outboxId, taskId, JSON.stringify({ taskId, eventId }), `task.queued:${taskId}:1`],
      );
      await client.query(
        ...
        [randomUUID(), input.requestedByUserId, taskId, JSON.stringify({ source: input.source, priority: input.priority })],
      );
