...
export class TaskService {
  ...
  async createHumanTask(input: CreateHumanTaskInput): Promise<TaskRecord> {
    ...
    if (input.type === "margin.analysis" || input.type === "pricing.simulation")
  ...
  async getTask(
    taskId: string,
  ): Promise<{ task: TaskRecord; events: readonly TaskEventRecord[] } | null> {
    const task = await this.repository.findTask?.(taskId);
    return task
      ? { task, events: (await this.repository.findEvents?.(taskId)) ?? [] }
      : null;
  }
}
