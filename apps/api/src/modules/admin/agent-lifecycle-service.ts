...
export interface AgentLifecycleRepository {
  ...
  revokeGrant(agentId: string, grantId: string, revokedByUserId: string): Promise<boolean>;
}

const timePattern = /^([01]\d|2[0-3]):[0-5]\d$/;
const transitions: Record<AgentLifecycleStatus, readonly AgentLifecycleStatus[]> = {
...
const schedules = (items: readonly AgentScheduleInput[]): void => {
  ...
  for (const item of items) {
    if (!Number.isInteger(item.weekday) || item.weekday < 0 || item.weekday > 6 || weekdays.has(item.weekday)) {
      throw new Error('schedule weekdays are invalid');
    }
    weekdays.add(item.weekday);
    if (!timePattern.test(item.workStart) || !timePattern.test(item.workEnd) || item.workStart >= item.workEnd) {
...
export class AgentLifecycleService {
  ...
  async appendVersion(input: AppendAgentVersionInput): Promise<AgentVersionRecord> {
    ...
    if (!['analytical', 'supervised', 'autonomous'].includes(input.trustCeiling)) throw new Error('trust ceiling is invalid');
    if (!['soft', 'hard'].includes(input.changeType)) throw new Error('change type is invalid');
  ...
  async transition(
    ...
    if (!transitions[from].includes(target)) throw new Error('lifecycle transition is invalid');
    if (!(await this.repository.transitionLifecycle(agentId, [from], target, changedByUserId))) {
  ...
  async replaceSchedule(
    ...
    text(changedByUserId, 'changedByUserId', 80);
    schedules(value);
    if (!(await this.repository.replaceSchedule(agentId, value, changedByUserId))) {
      throw new Error('agent was not found');
    }
  }

  async revokeGrant(agentId: string, grantId: string, revokedByUserId: string): Promise<void> {
    text(agentId, 'agentId', 80);
    text(grantId, 'grantId', 80);
    text(revokedByUserId, 'revokedByUserId', 80);
    if (!(await this.repository.revokeGrant(agentId, grantId, revokedByUserId))) {
      throw new Error('active grant was not found');
    }
  }
}

export const createAgentLifecycleService = (repository: AgentLifecycleRepository): AgentLifecycleService =>
