...
const parseSchedules = (
  value: unknown,
): CreateAgentInput["schedules"] | null => {
...
const parseInput = (body: unknown, userId: string): CreateAgentInput | null => {
  ...
  const parsedGrants = grants.map((grant) => ({
    toolCode: text(grant?.toolCode),
    accessLevel: text(grant?.accessLevel),
  }));
  if (
    parsedGrants.some(
      (grant) =>
        !grant.toolCode || !["read", "write"].includes(grant.accessLevel ?? ""),
    )
  )
    return null;
  return {
    ...
    name: values[2]!,
    title: values[3]!,
    primaryRole: values[4]!,
    basePrompt: values[5]!,
    mission: values[6]!,
    communicationStyle: values[7]!,
    modelProfile: values[8]!,
    responsibilities,
    restrictions,
    trustCeiling: trustCeiling as CreateAgentInput["trustCeiling"],
    createdByUserId: userId,
    schedules,
    grants: parsedGrants.map((grant) => ({
      toolCode: grant.toolCode!,
      accessLevel: grant.accessLevel as "read" | "write",
    })),
  };
};
...
export const registerAgentRoutes = (
  ...
  options: { authService: AuthService; agentService: AgentService },
): void => {
  server.post("/v1/admin/agents", async (request, reply) => {
    try {
      ...
      return reply
        .code(201)
        .send({ agent: await options.agentService.createAgent(input) });
    } catch {
