...
const required = (value: string, name: string, max: number): void => {
  if (!value.trim() || value.trim().length > max) throw new Error(`${name} is invalid`);
};
...
const validateSchedule = (schedules: readonly AgentScheduleInput[]): void => {
  ...
  for (const schedule of schedules) {
    ...
    if (weekdays.has(schedule.weekday)) throw new Error('schedule weekdays must be unique');
    weekdays.add(schedule.weekday);
    if (!timePattern.test(schedule.workStart) || !timePattern.test(schedule.workEnd) || schedule.workStart >= schedule.workEnd) {
...
const validateGrants = (grants: readonly AgentGrantInput[]): void => {
  ...
  for (const grant of grants) {
    ...
    if (tools.has(grant.toolCode)) throw new Error('tool grants must be unique');
...
export const createAgentService = (repository: AgentRepository): AgentService => new AgentService(repository);
