...
export class PostgresAgentLifecycleRepository implements AgentLifecycleRepository {
  ...
  async appendVersion(
    ...
    try {
      ...
      if (
        agent.rowCount !== 1 ||
        agent.rows[0]?.lifecycle_status === "archived"
      ) {
      ...
      if (!versionNumber)
        throw new Error("agent version number is unavailable");
      ...
      await client.query(
        "UPDATE agent SET active_version_id = $1, updated_at = now() WHERE id = $2",
        [versionId, input.agentId],
      );
      await recordAudit(
        client,
        input.createdByUserId,
        "agent_version_appended",
        "agent",
        input.agentId,
        {
          versionNumber,
  ...
  async replaceSchedule(
    ...
    try {
      ...
      const agent = await client.query(
        "SELECT 1 FROM agent WHERE id = $1 FOR UPDATE",
        [agentId],
      );
      if (agent.rowCount !== 1) {
        await client.query("ROLLBACK");
      ...
      await client.query("DELETE FROM agent_schedule WHERE agent_id = $1", [
        agentId,
      ]);
